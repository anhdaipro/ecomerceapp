...
class Thread(models.Model):
    ...
    def get_members(self):
        ...
        for user in self.participants.all():
            members.append({'user_id':user.id,'username':user.username,'avatar':user.profile.avatar.url})
        return members
...
class Message(models.Model):
    ...
    message_type=models.CharField(max_length=200,null=True,choices=message_type_choice,default='1')
    product=models.ForeignKey(to='shop.Item', on_delete=models.CASCADE,null=True,related_name='message_product')
    order=models.ForeignKey(to='checkout.Order', on_delete=models.CASCADE,null=True,related_name='message_order')
    date_created = models.DateTimeField(auto_now=True)
